...
# --- Paths & Google Sheet Info for Cabarrus County ---
...
SHEET_NAME = "Cabarrus eCourts Lawyers"
CREDENTIALS_FILE = os.path.join(base_dir, "credentials.json")

# --- Cabarrus County Cities ---
cabarrus_cities = r"CONCORD|KANNAPOLIS|HARRISBURG|MIDLAND|MT PLEASANT|MOUNT PLEASANT"
zip_code = r"\d{5}(?:-\d{4})?"
state = r"(?:North\s+Carolina|NC)"

# --- Step 1: Get latest folder ---
def get_latest_scraped_folder(base_path):
    pattern = re.compile(r"Cabarrus E-Courts Scraped File (\d{2}-\d{2}-\d{4})")
    dated_folders = []
    ...
    if not dated_folders:
        raise Exception("❌ No valid 'Cabarrus E-Courts Scraped File' folders found.")
...
def extract_property_address_from_text(text):
    ...
    patterns = [
        rf"\b\d{{3,6}}\s+[A-Z0-9\s#.'\-]+(?:,)?\s+(?:{cabarrus_cities})\s*,?\s*{state}\s*{zip_code}\b",
        rf"\b\d{{3,6}}\s+[A-Z0-9\s#.'\-]+(?:,)?\s+(?:{cabarrus_cities})\s+NC\s*{zip_code}\b",
        rf"\b\d{{1,6}}\s+[A-Z0-9\s#.'\-]+(?:,)?\s+(?:{cabarrus_cities})(?!\s+\w+)\s+NC\s+{zip_code}\b"
    ]
...
def is_courthouse_address(address):
    ...
    patterns = [
        r"77\s+union\s+st\.?",
        r"cabarrus\s+county\s+courthouse",
        r"clerk\s+of\s+(superior\s+)?court",
        r"room\s+\d{3,4}",
    ]
    city = r"concord"
    zipcodes = ["28025", "28026", "28027"]
    return any(re.search(p, address) for p in patterns) and city in address and any(z in address for z in zipcodes)
...
def update_sheet_with_addresses():
    ...
    print("\n✅ Finished updating Cabarrus County sheet.")
